...
/** ===== Config ===== */
const API_URL = "https://cakestories.ayatiworks.com/api/resto_lead.php";
...
const BROCHURE_URL = "/files/RestoCafe.pdf";
...
function MobileFlow() {
  ...
  /** saveAndGo("key", val) OR saveAndGo({patch}, nextStep) */
  ...
  const downloadBrochure = () => {
    ...
    a.href = BROCHURE_URL;
    a.download = "FreshlyMade-Brochure.pdf";
    ...
    >
      ...
      >
        ...
        {step === 1 && (
          <section className="pt-2 pb-6">
            <div className="flex items-center justify-between -mt-2 mb-8">
              <img src="/logo.jpg" alt="Cake Stories" className="h-15" />
            ...
            <button
              ...
              disabled={loading}
              className="mt-6 w-full rounded-xl bg-[#007AFF] text-lg text-white font-primary font-semibold py-4
            ...
            <div className="mt-4 ">
              <BackLink onBack={() => setStep(0)} />
            </div>
          </section>
        )}

        {/* Step 2: Role (SELECT ➜ CONTINUE) */}
        {step === 2 && (
          <SelectionStep
            logo
            title={`Mr. ${firstName(lead.name)},`}
            ...
            initialSelected={lead.role || ""}
            onBack={() => setStep(1)}
            onContinue={async (value) => saveAndGo("role", value)}
            loading={loading}
            err={err}
          />
        )}
        ...
        {step === 3 && lead.role === "Business Owner" && (
          <SelectionStep
            title={`Mr. ${firstName(lead.name)},`}
            subtitle="What kind of business do you currently operate?"
            options={[
              { label: "Café or Restaurant", value: "Café or Restaurant", img: "/icon-1.png" },
              { label: "Bakery or Cloud Kitchen", value: "Bakery or Cloud Kitchen", img: "/icon-2.png" },
              { label: "Retail or FMCG Outlet", value: "Retail or FMCG Outlet", img: "/icon-3.png" },
              { label: "Other", value: "Other", img: "/icon-4.png" },
            ]}
            render="icons"
            initialSelected={lead.business_type || ""}
            onBack={() => setStep(2)}
            onContinue={async (value) => saveAndGo("business_type", value)}
            loading={loading}
            err={err}
          />
        )}
        ...
        {step === 4 && lead.role === "Business Owner" && (
          <SelectionStep
            logo
            title={`Mr. ${firstName(lead.name)},`}
            subtitle="What’s your available investment range for expansion?"
            options={[
              { label: "₹45 L – ₹60 L", value: "₹45 L – ₹60 L" },
              // { label: "₹60 L +", value: "₹60 L +" },
              { label: "Above ₹60 L", value: "Above ₹60 L" },
            ]}
            initialSelected={lead.investment_range || ""}
            onBack={() => setStep(3)}
            onContinue={async (value) => saveAndGo("investment_range", value)}
            loading={loading}
            err={err}
          />
        )}
        ...
        {step === 5 && lead.role === "Business Owner" && (
          <SelectionStep
            logo
            title={`Mr. ${firstName(lead.name)},`}
            subtitle="When is the best time for our team to reach you?"
            options={[
            ...
            initialSelected={lead.contact_time || ""}
            onBack={() => setStep(4)}
            onContinue={async (value) => saveAndGo({ contact_time: value }, 6)}
            loading={loading}
            err={err}
          />
        )}
        ...
        {step === 3 && lead.role === "Investor" && (
          <SelectionStep
            logo
            title={`Mr. ${firstName(lead.name)},`}
            subtitle="What type of franchise are you most interested in?"
            options={[
            ...
            initialSelected={lead.investor_interest || ""}
            onBack={() => setStep(2)}
            onContinue={async (value) => saveAndGo("investor_interest", value)}
            loading={loading}
            err={err}
          />
        )}
        ...
        {step === 4 && lead.role === "Investor" && (
          <SelectionStep
            logo
            title={`Mr. ${firstName(lead.name)},`}
            subtitle="What’s your investment budget?"
            options={[
              { label: "₹45 L – ₹60 L", value: "₹45 L – ₹60 L" },
              // { label: "₹60 L +", value: "₹60 L +" },
              { label: "Above ₹60 L", value: "Above ₹60 L" },
            ]}
            initialSelected={lead.investor_budget || ""}
            onBack={() => setStep(3)}
            onContinue={async (value) => saveAndGo("investor_budget", value)}
            loading={loading}
            err={err}
          />
        )}
        ...
        {step === 5 && lead.role === "Investor" && (
          <SelectionStep
            logo
            title={`Mr. ${firstName(lead.name)},`}
            subtitle="When is a good time for us to contact you?"
            options={[
              { label: "Morning (10 AM – 12 PM)", value: "Morning (10 AM – 12 PM)" },
              { label: "Afternoon (12 PM – 4 PM)", value: "Afternoon (12 PM – 4 PM)" },
              { label: "Evening (4 PM – 7 PM)", value: "Evening (4 PM – 7 PM)" },
            ]}
            initialSelected={lead.investor_contact_time || ""}
            onBack={() => setStep(4)}
            onContinue={async (value) => saveAndGo({ investor_contact_time: value }, 6)}
            loading={loading}
            err={err}
          />
        )}
        ...
        {step === 3 && lead.role === "Exploring" && (
          <SelectionStep
            logo
            title={`Mr. ${firstName(lead.name)},`}
            subtitle="What kind of business are you most excited to start?"
            options={[
            ...
            initialSelected={lead.exploring_kind || ""}
            onBack={() => setStep(2)}
            onContinue={async (value) => saveAndGo("exploring_kind", value)}
            loading={loading}
            err={err}
          />
        )}
        ...
        {step === 4 && lead.role === "Exploring" && (
          <SelectionStep
            logo
            title={`Mr. ${firstName(lead.name)},`}
            subtitle="What’s your estimated investment capacity?"
            options={[
              { label: "₹45 L – ₹60 L", value: "₹45 L – ₹60 L" },
              // { label: "₹60 L +", value: "₹60 L +" },
              { label: "Above ₹60 L", value: "Above ₹60 L" },
            ]}
            initialSelected={lead.exploring_budget || ""}
            onBack={() => setStep(3)}
            onContinue={async (value) => saveAndGo("exploring_budget", value)}
            loading={loading}
            err={err}
          />
        )}
        ...
        {step === 5 && lead.role === "Exploring" && (
          <SelectionStep
            logo
            title={`Mr. ${firstName(lead.name)},`}
            subtitle="When are you planning to start your business?"
            options={[
            ...
            initialSelected={lead.exploring_timeline || ""}
            onBack={() => setStep(4)}
            onContinue={async (value) => saveAndGo({ exploring_timeline: value }, 6)}
            loading={loading}
            err={err}
          />
        )}
...
function SelectionStep({
  ...
  loading = false,
  err = "",
  render = "text", // "text" | "icons"
}) {
  const [selected, setSelected] = useState(initialSelected);

  return (
    <section>
      {logo && <img src="/logo.jpg" alt="" className="h-12 mb-8" />}
      {title && <h2 className="text-3xl sm:text-4xl font-primary font-bold mb-2">{title}</h2>}
      {subtitle && (
        <p className="text-lg sm:text-xl font-secondary text-black/60 mt-3 mb-8">{subtitle}</p>
      )}

      {render === "icons" ? (
        <IconSelectList options={options} selected={selected} onSelect={setSelected} />
      ) : (
      ...
      <div className="flex items-center justify-between mt-8">
        ...
        <button
          ...
          className={[
            "rounded-lg text-white text-base sm:text-lg font-semibold px-5 py-3 transition",
            !selected || loading
              ? "bg-[#007AFF] opacity-50 cursor-not-allowed"
              : "bg-[#007AFF] hover:brightness-110",
          ].join(" ")}
        >
          {loading ? "Saving…" : "Continue"}
        </button>
...
/* ================= Lists ================= */
function SelectList({ options, selected, onSelect }) {
  return (
        ...
        return (
          <li key={opt.value}>
            <button
              ...
              className={[
                "w-full rounded-xl border font-secondary bg-white px-4 py-4 sm:py-5 text-left",
                "text-lg sm:text-2xl font-semibold transition",
                active
                  ? "border-[#007AFF] shadow-[0_0_0_3px_rgba(0,122,255,0.15)] text-slate-900"
...
function IconSelectList({ options, selected, onSelect }) {
  return (
    <ul className="space-y-4">
      {options.map((opt) => {
        const active = selected === opt.value;
        return (
          <li key={opt.value}>
            <button
              ...
              onClick={() => onSelect(opt.value)}
              className={[
                "w-full rounded-xl font-secondary px-4 py-4 text-left border transition bg-white",
            ...
            >
              {opt.img && <img src={opt.img} alt="" className="h-10 w-10 object-contain" />}
